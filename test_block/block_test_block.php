 ...
 * @copyright  Abhijit Sen<abhijitsentiku@gmail.com>
 */
...
class block_test_block extends block_list {
    ...
    public function init() {
        ...
        $htmldisplay .= get_string('pluginname', 'block_test_block');
    ...
    private function get_display_data($moddetail, $roleid) {
        global $USER;
        ...
        $htmldisplay = '';
        $htmldisplay .= html_writer::start_tag('div',  array('class' => 'activityinfo'));
    ...
    private function get_activity_status($cmid, $userid) {
        global $DB;
        ...
        if (!empty($activitystatus)) {
            if ($activitystatus->completionstate == 1) {
                $status = get_string('completed', 'block_test_block');
            } else if ($activitystatus->completionstate == 2) {
                $status = get_string('completepass', 'block_test_block');
            } else if ($activitystatus->completionstate == 3) {
                $status = get_string('completefail', 'block_test_block');
            }
        } else {
            $status = get_string('notstarted', 'block_test_block');
        }
